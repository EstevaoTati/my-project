...
on a branch cut from `main`. The Mwinda site at `/` is untouched apart from
three additive entries (a redirect pair, a header block, a sitemap line) and
one widening of `autoplay=()` to `autoplay=(self)`.
